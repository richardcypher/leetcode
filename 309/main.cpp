/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2015年11月25日 星期三 09时56分53秒
 ************************************************************************/
...
int maxProfit1(vector<int>& prices) {
    ...
    for(s = 1; s < days; s++) {
        for(i = 0; i < days - s; i++) {
            j = i + s;
            if(minprice[i] > prices[j])
                minprice[i] = prices[j];
            p[i][j] = p[i][j - 1];
            if(prices[j] - minprice[i] > p[i][j - 1])
                p[i][j] = prices[j] - minprice[i];
            if(s >= 4) {
                ...
                for(d = i; d <= j; d++) {
                    if(d <= i + 1)
                        p[i][j] = p[i][j] < p[i + 2][j] ? p[i + 2][j] : p[i][j];
                    else if(d >= j - 1)
                        p[i][j] = p[i][j] < p[i][j - 2] ? p[i][j - 2] : p[i][j];
                    else if(p[i][j] < p[i][d - 1] + p[d + 1][j])
                        p[i][j] = p[i][d - 1] + p[d + 1][j]; 
                }
            }
        }
    }
...
int maxProfit(vector<int>& prices) {
    int prev_buy, buy(INT_MIN), sell = 0, prev_sell;
    for(int i = 0; i < prices.size(); i++) {
        prev_buy = buy;
        buy = prev_sell - prices[i] < buy ? buy : (prev_sell - prices[i]);
        prev_sell = sell;
        sell = prev_buy + prices[i]  < sell ? sell : prev_buy + prices[i];
    }
    return sell;
}
