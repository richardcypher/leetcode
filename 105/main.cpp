...
TreeNode* buildHelper(vector<int>& preorder, vector<int>& inorder, int i1, int j1, int i2, int j2) {
    ...
    cout<<index<<endl;
    TreeNode* left = buildHelper(preorder, inorder, i1 + 1, j1, index, index - 1);
    TreeNode* right = buildHelper(preorder, inorder,  index + 1,  index + 1, i2, j2);
    root->left = left;
    root->right = right;
    return root;
}
